import { gameBus } from '../../1-kernel/core/EventBus';
import { Vector3 } from '../../1-kernel/core/Entity';
import {
  NetworkAbilityRequest,
  NetworkHitValidationRequest,
  NetworkInputCommand,
} from './NetworkRuntimeContracts';
import { applyInput, applyRotation, resolveStatusMovementModifier } from './NetworkMovementPrediction';
import type { NetworkSyncBinding } from './NetworkSyncSystemImpl';
import { DEFAULT_MOVEMENT_TUNING_CONFIG } from './MovementTuningConfig';

export function registerPlayer(context: any, binding: NetworkSyncBinding, options: { local?: boolean } = {}): void {
  const networkEntityId = binding.networkEntityId ?? binding.playerId;
  const previousNetworkEntityId = context.networkEntityIdsByPlayer.get(binding.playerId);
  if (previousNetworkEntityId && previousNetworkEntityId !== networkEntityId) {
    context.replicationSystem.unregisterBinding(previousNetworkEntityId);
  }

  context.bindings.set(binding.playerId, binding);
  context.networkEntityIdsByPlayer.set(binding.playerId, networkEntityId);
  context.networkEntityIdRegistrar?.reserveHandleForPlayer(binding.playerId);
  context.tryRegisterNetworkEntityMapping(binding.playerId, networkEntityId);
  context.replicationSystem.registerBinding(networkEntityId, {
    entity: binding.entity,
    instance: binding.replicatedInstance,
    velocityProvider: () => context.getVelocity(binding.playerId),
  });
  if (context.movementState.has(binding.playerId)) {
    console.log('[NetworkSyncSystem] Resetting movement state for player bind', {
      playerId: binding.playerId,
      entityId: binding.entity.id,
      networkEntityId,
    });
  }
  context.movementState.set(binding.playerId, {
    ...context.createMovementRuntimeState(context.tick),
    groundHeight: binding.entity.getPosition().y,
    lastCorrectionTick: context.tick,
  });
  context.spatialPartition.updateEntry(binding.entity.id, binding.entity.getPosition(), {
    radius: binding.collisionRadius ?? 0.8,
    tags: [binding.entity.type, options.local ? 'local' : 'networked'],
    isActive: binding.entity.isActive,
    lastUsedTime: binding.entity.lastUsedTime,
  });

  if (options.local) {
    context.localPlayerId = binding.playerId;
    context.movementDebugState.lastBindingResetAt = Engine.time.now();
    context.movementDebugState.lastBindingResetDetails = {
      playerId: binding.playerId,
      entityId: binding.entity.id,
      networkEntityId,
    };
    context.movementDebugState.lastInputSource = 'bindLocalPlayer';
    context.movementDebugState.timestamp = Engine.time.now();
  }
}

export function bindLocalPlayer(context: any, playerId: string, entity: any, options: Omit<NetworkSyncBinding, 'playerId' | 'entity'> = {}): void {
  const existingBinding = context.bindings.get(playerId);
  const newNetworkEntityId = options.networkEntityId ?? playerId;
  const previousNetworkEntityId = context.networkEntityIdsByPlayer.get(playerId);
  const isLocalBindingRebind = existingBinding && (
    existingBinding.entity !== entity
    || previousNetworkEntityId !== newNetworkEntityId
  );

  if (isLocalBindingRebind) {
    if (previousNetworkEntityId) {
      context.replicationSystem.unregisterBinding(previousNetworkEntityId);
    }
    context.bindings.delete(playerId);
    context.networkEntityIdsByPlayer.delete(playerId);
    context.pendingInputs.delete(playerId);
    context.pendingMovementIntent.delete(playerId);
    context.lastProcessedInputSeq.delete(playerId);
    context.movementState.delete(playerId);
    context.statusMovementModifiers.delete(playerId);
    context.derivedStatusMovementModifiers.delete(playerId);
    context.debugStatusMovementModifiers.delete(playerId);
  }

  if (context.localPlayerId && context.localPlayerId !== playerId) {
    const previousPlayerId = context.localPlayerId;
    const previousBinding = context.bindings.get(previousPlayerId);
    if (previousBinding?.entity === entity) {
      const previousNetworkEntityId = context.networkEntityIdsByPlayer.get(previousPlayerId);
      if (previousNetworkEntityId) {
        context.replicationSystem.unregisterBinding(previousNetworkEntityId);
      }
      context.bindings.delete(previousPlayerId);
      context.networkEntityIdsByPlayer.delete(previousPlayerId);
      context.pendingInputs.delete(previousPlayerId);
      context.pendingMovementIntent.delete(previousPlayerId);
      context.lastProcessedInputSeq.delete(previousPlayerId);
      context.movementState.delete(previousPlayerId);
      context.statusMovementModifiers.delete(previousPlayerId);
      context.derivedStatusMovementModifiers.delete(previousPlayerId);
      context.debugStatusMovementModifiers.delete(previousPlayerId);
    }
  }

  registerPlayer(context, {
    playerId,
    entity,
    movementSpeed: options.movementSpeed,
    acceleration: options.acceleration,
    collisionRadius: options.collisionRadius,
    replicatedInstance: options.replicatedInstance,
    networkEntityId: options.networkEntityId ?? playerId,
  }, { local: true });
}

export function setAbilityValidator(context: any, validator: (request: NetworkAbilityRequest) => boolean | string): void {
  context.abilityValidator = validator;
}

export function queueLocalInput(context: any, input: Record<string, unknown>): NetworkInputCommand | null {
  const localPlayerId = context.ensureLocalPlayerBinding();
  if (!localPlayerId) return null;

  context.liveInputs.set(localPlayerId, { ...input });

  const command: NetworkInputCommand = {
    playerId: localPlayerId,
    seq: context.networkManager.nextInputSequence(),
    tick: context.lastServerTick,
    timestamp: Engine.time.now(),
    input,
  };
  const queue = context.pendingInputs.get(localPlayerId) ?? [];
  queue.push(command);

  const maxBufferedInputs = Math.max(4, Math.ceil(context.tickRate * context.historySeconds * 2));
  while (queue.length > maxBufferedInputs) {
    const dropped = queue.shift();
    console.warn('[NetworkSyncSystem] Input queue overflow, dropped old input', {
      playerId: localPlayerId,
      droppedSeq: dropped?.seq,
      droppedTick: dropped?.tick,
      queueLen: queue.length,
      maxBuffered: maxBufferedInputs,
      lastServerTick: context.lastServerTick,
      lastAppliedSnapshotTick: context.lastAppliedSnapshotTick,
      ackInputSeq: context.lastProcessedInputSeq.get(localPlayerId),
    });
  }
  context.pendingInputs.set(localPlayerId, queue);
  context.movementDebugState.lastQueuedInputCommand = command;
  context.movementDebugState.pendingInputCount = queue.length;
  context.movementDebugState.lastInputSource = 'queueLocalInput';
  context.movementDebugState.lastMovementIntentSource = null;
  context.movementDebugState.timestamp = Engine.time.now();

  gameBus.emit('playerInput', { ...command, input: { ...input } });
  gameBus.emit('INPUT_BUFFERED', {
    playerId: localPlayerId,
    seq: command.seq,
    tick: command.tick,
    timestamp: command.timestamp,
    input: { ...input },
  });

  const sendPath = (context.authorityMode === 'remote' && context.commandSink && context.playerInitReady)
    ? 'commandSink'
    : 'networkManager';

  if (sendPath === 'commandSink') {
    context.commandSink!(command);
  } else {
    context.networkManager.sendInputCommand(command);
  }

  if (context.authorityMode === 'remote' && context.commandSink && !context.playerInitReady) {
    console.warn('[NetworkSyncSystem] Remote command sink not ready, sending via networkManager instead', {
      playerId: localPlayerId,
      seq: command.seq,
      tick: command.tick,
      pendingQueued: queue.length,
      authorityMode: context.authorityMode,
      playerInitReady: context.playerInitReady,
    });
  }

  gameBus.emit('COMMAND_SENT', {
    playerId: localPlayerId,
    seq: command.seq,
    tick: command.tick,
    timestamp: command.timestamp,
    input: { ...input },
  });
  return command;
}

export function stepLocalInput(context: any, input: Record<string, unknown>, dt: number): void {
  const localPlayerId = context.ensureLocalPlayerBinding();
  if (!localPlayerId) return;
  context.liveInputs.set(localPlayerId, { ...input });
  const binding = context.bindings.get(localPlayerId);
  if (!binding) return;

  context.movementDebugState.lastLiveInput = { ...input };
  context.movementDebugState.lastLiveInputDt = dt;
  context.movementDebugState.lastInputSource = 'stepLocalInput';
  context.movementDebugState.timestamp = Engine.time.now();

  if (context.authorityMode === 'remote' && context.remotePredictionMode === 'rotation-only') {
    applyRotation({
      binding,
      input,
      movementState: context.movementState,
      currentTick: context.tick,
    });
    return;
  }

  const runtime = context.movementState.get(localPlayerId) ?? {
    ...context.createMovementRuntimeState(),
    groundHeight: binding.entity.getPosition().y,
    lastCorrectionTick: context.tick,
  };
  const yaw = typeof input.yaw === 'number' ? input.yaw : binding.entity.getRotation().y;
  const pitch = typeof input.pitch === 'number' ? input.pitch : binding.entity.getRotation().x;
  runtime.isCrouching = input.crouch === true;
  binding.entity.setRotation({
    ...binding.entity.getRotation(),
    x: pitch,
    y: yaw,
  });
  context.movementState.set(localPlayerId, runtime);

  context.localStepAccumulator = Math.min(
    context.localStepAccumulator + Math.max(1 / 240, dt),
    context.localInputFixedStep * 4,
  );

  try {
    while (context.localStepAccumulator >= context.localInputFixedStep) {
      applyInput({
        binding,
        input,
        dt: context.localInputFixedStep,
        currentTick: context.tick,
        movementState: context.movementState,
        collisionResolver: context.collisionResolver,
        movementFeelDebugConfigs: context.movementFeelDebugConfigs,
        resolvedMovementTuningScratch: context.resolvedMovementTuningScratch,
        pendingMovementIntent: context.pendingMovementIntent,
        resolveStatusMovementModifier: (playerId: string) => resolveStatusMovementModifier(
          playerId,
          context.statusMovementModifiers,
          context.derivedStatusMovementModifiers,
          context.debugStatusMovementModifiers,
        ),
        localPlayerId: context.localPlayerId,
        spatialPartition: context.spatialPartition,
      });
      context.localStepAccumulator -= context.localInputFixedStep;
    }
  } catch (error) {
    console.error('[NetworkSyncSystem] stepLocalInput crash', { playerId: binding.playerId, input, error });
  }

  gameBus.emit('playerInput', {
    playerId: localPlayerId,
    seq: -1,
    tick: context.tick,
    timestamp: Engine.time.now(),
    input: { ...input },
  });
}

export function setLiveLocalInput(context: any, input: Record<string, unknown>): void {
  const localPlayerId = context.ensureLocalPlayerBinding();
  if (!localPlayerId) return;
  context.liveInputs.set(localPlayerId, { ...input });
}

export function clearLiveLocalInput(context: any): void {
  const localPlayerId = context.localPlayerId;
  if (!localPlayerId) return;
  context.liveInputs.delete(localPlayerId);
}

export function requestAbilityActivation(context: any, abilityId: string, payload?: Record<string, unknown>): boolean {
  const localPlayerId = context.ensureLocalPlayerBinding();
  if (!localPlayerId) return false;
  const request: NetworkAbilityRequest = {
    playerId: localPlayerId,
    abilityId,
    timestamp: Engine.time.now(),
    payload,
  };
  gameBus.emit('abilityActivationRequested', {
    entityId: localPlayerId,
    abilityId,
    payload,
  });
  context.networkManager.sendAbilityRequest(request);
  return true;
}

export function requestHitscanValidation(context: any, request: Omit<NetworkHitValidationRequest, 'shooterId' | 'timestamp'>): boolean {
  const localPlayerId = context.ensureLocalPlayerBinding();
  if (!localPlayerId) return false;
  context.networkManager.sendHitValidationRequest({
    ...request,
    shooterId: localPlayerId,
    timestamp: Engine.time.now(),
  });
  return true;
}

export function forceLocalState(
  context: any,
  position: Vector3,
  rotation: Vector3,
  velocity?: Vector3,
  options: { clearPendingInputs?: boolean } = {},
): void {
  const localPlayerId = context.ensureLocalPlayerBinding();
  if (!localPlayerId) return;
  const binding = context.bindings.get(localPlayerId);
  if (!binding) return;

  binding.entity.setPosition(position);
  binding.entity.setRotation(rotation);
  const runtime = context.movementState.get(localPlayerId) ?? {
    ...context.createMovementRuntimeState(),
    groundHeight: position.y,
    lastCorrectionTick: context.tick,
  };
  runtime.velocity = velocity ? cloneVector(velocity) : { x: 0, y: 0, z: 0 };
  runtime.groundHeight = position.y;
  context.movementState.set(localPlayerId, runtime);

  if (options.clearPendingInputs !== false) {
    context.pendingInputs.set(localPlayerId, []);
    context.movementDebugState.pendingInputCount = 0;
    context.movementDebugState.lastInputSource = 'forceLocalState';
    context.movementDebugState.timestamp = Engine.time.now();
  }

  context.spatialPartition.updateEntry(binding.entity.id, binding.entity.getPosition(), {
    radius: binding.collisionRadius ?? 0.8,
    tags: [binding.entity.type, 'local'],
    isActive: binding.entity.isActive,
    lastUsedTime: binding.entity.lastUsedTime,
  });
}

export function clearPendingInputs(context: any, playerId?: string): void {
  const targetId = playerId ?? context.localPlayerId;
  if (!targetId) return;
  context.pendingInputs.set(targetId, []);
  context.pendingMovementIntent.delete(targetId);
  context.lastProcessedInputSeq.delete(targetId);
}

function cloneVector(vector: Vector3 | undefined): Vector3 {
  if (!vector) return { x: 0, y: 0, z: 0 };
  return { x: vector.x, y: vector.y, z: vector.z };
}
